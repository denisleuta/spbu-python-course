...
from project.roulette import RouletteGame, ConservativeBot, AggressiveBot, RandomBot, Bet
...
def test_spin_wheel(setup_game):
    game = setup_game
    number, color = game.spin_wheel()
    assert 0 <= number <= 36
    assert color in game.COLORS

def test_budget_update(setup_game):
    game = setup_game
    bot = game.bots[0]
...
def test_bet_evaluation(setup_game):
    game = setup_game
    winning_number = 5
    winning_color = 'Red'
...
def test_game_progression(setup_game):
    game = setup_game
    game.play()
    assert game.round > 1
